/*******************************************************************
* Autor: Alexey Vadimovich
* Annotation: This program prints a message to the screen in the console
* Date: 23-09-2020
********************************************************************/

#include <iostream>
#include <cstdlib>

int main()
{
	std::cout << std::endl;
	std::cout << std::endl;

	std::cout << "HHHH       HHHH EEEEEEEEE  LLLLL            LLLLL           OOOOOOOOOOOOOO !!!! !!!! !!!!   " << std::endl;
	std::cout << "HHHH       HHHH EEEEEEEEE  LLLLL            LLLLL           OOOOOOOOOOOOOO !!!! !!!! !!!!   " << std::endl;
	std::cout << "HHHH       HHHH EEEEEEEEE  LLLLL            LLLLL           OOOOOOOOOOOOOO !!!! !!!! !!!!   " << std::endl;
	std::cout << "HHHH       HHHH EEE        LLLLL            LLLLL           OOO        OOO !!!! !!!! !!!!   " << std::endl;
	std::cout << "HHHH       HHHH EEE        LLLLL            LLLLL           OOO        OOO !!!! !!!! !!!!   " << std::endl;
	std::cout << "HHHH       HHHH EEEEEE     LLLLL            LLLLL           OOO        OOO !!!! !!!! !!!!   " << std::endl;
	std::cout << "OOOHHHHHHHHOOOH EEEEEE     LLLLL            LLLLL           OOO        OOO !!!! !!!! !!!!   " << std::endl;
	std::cout << "HHHH       HHHH EEEEEE     LLLLL            LLLLL           OOO        OOO !!!! !!!! !!!!   " << std::endl;
	std::cout << "HHHH       HHHH EEE        LLLLL            LLLLL           OOO        OOO !!!! !!!! !!!!   " << std::endl;
	std::cout << "HHHH       HHHH EEE        LLLLL            LLLLL           OOO        OOO !!!! !!!! !!!!   " << std::endl;
	std::cout << "HHHH       HHHH EEEEEEEEE  LLLLLLLLLLLLLLL  LLLLLLLLLLLLLLL OOOOOOOOOOOOOO				          " << std::endl;
	std::cout << "HHHH       HHHH EEEEEEEEE  LLLLLLLLLLLLLLL  LLLLLLLLLLLLLLL OOOOOOOOOOOOOO !!!! !!!! !!!!   " << std::endl;
	std::cout << "HHHH       HHHH EEEEEEEEE  LLLLLLLLLLLLLLL  LLLLLLLLLLLLLLL OOOOOOOOOOOOOO !!!! !!!! !!!!   " << std::endl;

	std::cout << std::endl;
	std::cout << std::endl;
	system("pause");
	return 0;
}